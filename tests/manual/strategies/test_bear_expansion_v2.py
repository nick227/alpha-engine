"""
Test Bear Expansion Strategy V2

Refined risk structure:
- Precision entry band: 0.2-0.3 only
...
from app.trading.bear_expansion_trader_v2 import BearExpansionTraderV2, BearExpansionConfigV2
...
def test_bear_expansion_v2(df):
    """Test the refined bear expansion strategy"""
    
    print("\n=== BEAR EXPANSION STRATEGY V2 TEST ===")
    print("Refined risk structure: Precision entry band + tighter stops")
    
    # Initialize trader with refined config
    config = BearExpansionConfigV2(
        min_position_in_range=0.2,         # Minimum 20% of range
        max_position_in_range=0.3,         # Maximum 30% of range
        base_risk_per_trade=0.015,        # 1.5% risk per trade (reduced)
        atr_stop_multiplier=1.5,          # Tighter stop at 1.5x ATR
        atr_target_multiplier=3.0,        # Target at 3x ATR
        max_concurrent_positions=3,        # Reduced from 5
        max_portfolio_heat=0.10,          # Reduced to 10%
        max_positions_per_sector=1,       # Prevent sector clustering
        use_trailing_stop=False,
        max_hold_days=10
    )
    
    trader = BearExpansionTraderV2(initial_capital=100000.0, config=config)
    ...
    print(f"  Stop Multiplier: {config.atr_stop_multiplier}x ATR")
    ...
    # Compare to V1
    print(f"\n=== V1 vs V2 COMPARISON ===")
    v1_return = 0.7  # From previous test
    v1_dd = 35.4
    v1_sharpe = 0.41
    
    print(f"  V1: {v1_return:+.1f}% return, {v1_dd:.1f}% DD, {v1_sharpe:.2f} Sharpe")
    print(f"  V2: {total_return:+.1f}% return, {max_dd:.1f}% DD, {sharpe:.2f} Sharpe")
    print(f"  Improvement: {total_return - v1_return:+.1f}% return, {v1_dd - max_dd:+.1f}% DD reduction")
    ...
    if len(df) > 0:
        ...
        print(f"  Bear Expansion V2 Return: {total_return:+.1f}%")
...
def main():
    ...
    print("Bear Expansion Strategy V2 Test")
    print("Refined risk structure: Precision entry band + tighter stops\n")
    ...
    results = test_bear_expansion_v2(df)
