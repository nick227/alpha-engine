...
def compute_return_from_prices(ticker, entry_date, exit_date):
    ...
    # In a real system, you'd have separate open/close prices
    exit_query = """
...
def test_variant1_final():
    """Variant 1 with manual return calculation (no outcome reuse)"""
    
    simulator = PortfolioSimulator("data/alpha.db")
    simulator.connect()
    
    # Find -2% drops (same signal definition)
    price_query = """
    SELECT 
    ...
    return len(drop_signals), win_rate, portfolio_return, daily_returns

def test_variant1_time_shift_final():
    """Time-shift test with manual return calculation"""
    ...
    print(f"\n=== VARIANT 1 TIME-SHIFT - FINAL FIX ===")
...
if __name__ == "__main__":
    print("=== TESTING FINAL FIX: MANUAL RETURN CALCULATION ===")
    print("Removing all usage of prediction_outcomes.return_pct\n")
    
    signals, winrate, portfolio, returns = test_variant1_final()
    normal, shifted, normal_wr, shifted_wr = test_variant1_time_shift_final()
    ...
    if shifted <= 0.01:
        ...
        print("System now provides honest signal evaluation")
    else:
        print(f"\nSTILL LEAKING: Need further investigation")
