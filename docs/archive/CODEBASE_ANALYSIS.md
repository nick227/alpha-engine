...
- `docs/archive/ARCHITECTURE.md`: canonical flow: raw events → scored events → MRA outcomes → strategies → predictions → outcomes → ranking.
- `docs/archive/ROADMAP.md`: POC focuses on dual track (news + quant), prediction-first evaluation, backtest “horse racing”, Streamlit comparison dashboard; next is paper-mode runner + more technical families + hybrid text+technical + calibration/ranking metrics; later Postgres/SaaS migration.
