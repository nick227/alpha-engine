...
Remove partial exits to eliminate accounting ambiguity.
Clean, undeniable concentration metrics.
"""
...
class SimpleValidationPipeline:
    """
    Simple validation with full entry/exit only - no partials.
    """
    ...
    def load_data(self):
        ...
        print("Loading historical data for simple validation...")
    ...
    def generate_simple_trades(self):
        """
        Generate simple trades with FULL entry/exit only - no partials.
        """
        
        print("Generating simple trades with FULL entry/exit only...")
        ...
        for current_date in trading_days:
            ...
            for pos_id, exit_price, exit_reason in positions_to_close:
                ...
                # Calculate P&L for short position (FULL exit)
                pnl = position['quantity'] * (position['entry_price'] - exit_price)
                capital += pnl
                
                # Record trade (SIMPLE - no partials)
                trades.append({
        ...
        print(f"Generated {len(trades)} simple trades (no partials)")
        
        return trades
    
    def calculate_clean_metrics(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Calculate metrics from simple trades (no partials).
        """
        ...
        # Top trade contributions (CLEAN calculation)
        ...
        # Calculate contribution percentages (CLEAN denominator)
        ...
        # Sector contributions (CLEAN calculation)
    ...
    def run_simple_validation(self):
        """
        Run simple validation with clean accounting.
        """
        
        print("Simple Validation Pipeline")
        print("FULL entry/exit only - no partials, no scaling complexity\n")
    ...
    def print_clean_results(self, metrics: Dict[str, Any]):
        """
        Print clean results with definitive validation.
        """
        ...
        if metrics['top_5_contribution'] <= 50:
            ...
            print("   Convex spike system - needs concentration controls")
        ...
        print(f"\n🔥 DEFINITIVE TRUTH EXTRACTION COMPLETE")
        print(f"   Accounting is clean - metrics are undeniable")


def main():
    """Main simple validation function"""
    
    validator = SimpleValidationPipeline()
    metrics = validator.run_simple_validation()
    
    return validator, metrics


if __name__ == "__main__":
    validator, metrics = main()
