...
Fix the double-counting issue with ONE row per position.
"""
...
class FixedAggregationPipeline:
    """
    Fixed aggregation with canonical position-level accounting.
    """
    ...
    def load_data(self):
        ...
        print("Loading historical data for fixed aggregation...")
    ...
    def generate_raw_exits(self):
        """
        Generate raw exit events (including partials) for aggregation.
        """
        
        print("Generating raw exit events for aggregation...")
        ...
        for current_date in trading_days:
            ...
            for pos_id, position in positions.items():
                ...
                # Profit scaling: TP1 at +1R (PARTIAL)
            ...
            for pos_id, exit_price, exit_reason, current_r in positions_to_close:
                ...
                # Calculate P&L for this exit (INCREMENTAL)
                if exit_reason == "tp1_partial":
                    # Partial exit: calculate incremental P&L for this exit only
                    pnl = position['quantity'] * 0.5 * (position['entry_price'] - exit_price)
                    remaining_quantity = position['quantity'] * 0.5  # Keep half position
    ...
    def calculate_fixed_metrics(self, df_positions: pd.DataFrame) -> Dict[str, Any]:
        """
    ...
    def run_fixed_aggregation(self):
        """
        Run fixed aggregation with proper position-level accounting.
        """
        
        print("Fixed Aggregation Pipeline")
        print("ONE row per position, ONE P&L per position\n")
        ...
        # Generate raw exit events
        raw_exits = self.generate_raw_exits()
        
        # Aggregate to position-level trades
        df_positions = self.aggregate_to_positions(raw_exits)
        
        # Calculate fixed metrics
        metrics = self.calculate_fixed_metrics(df_positions)
        
        # Print results
        self.print_fixed_results(metrics)
        
        return metrics
    
    def print_fixed_results(self, metrics: Dict[str, Any]):
        """
        Print fixed metrics with validation checks.
        """
        ...
        if metrics['top_5_contribution'] <= 50:
            ...
            print("   Real controlled edge achieved")
        elif metrics['top_5_contribution'] <= 80:
            print("⚠️  MODERATE CONCENTRATION")
            print("   Some concentration, but reasonable")
        else:
...
def main():
    """Main fixed aggregation function"""
    
    fixer = FixedAggregationPipeline()
    metrics = fixer.run_fixed_aggregation()
