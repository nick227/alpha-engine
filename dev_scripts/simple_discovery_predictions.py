...
# Simple approach: manually create predictions from discovery candidates
conn = sqlite3.connect("data/alpha.db")

# Get discovery candidates with highest scores per strategy
...
for candidate in candidates:
    ...
    if len(strategy_predictions[strategy_id]) < 3:
        direction = "UP" if candidate[2] > 0.7 else "DOWN"  # candidate[2] = score
        confidence = float(candidate[2])  # candidate[2] = score
        
        prediction_id = f"disc_{candidate[1]}_{datetime.now().strftime('%Y%m%d%H%M%S')}"  # candidate[1] = symbol
        ...
        """, (
            ...
            candidate[1],                                     # 4. ticker (candidate[1] = symbol)
        ...
        strategy_predictions[strategy_id].append(candidate[1])  # candidate[1] = symbol
