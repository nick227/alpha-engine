...
def test_valid_symbol_backtest():
    """Test backtest with known valid symbols like SPY, QQQ, NVDA."""
    repo = AlphaRepository("data/alpha.db")
    
    # Use an earlier date to ensure we have price data for 5-day horizon
    target_date = datetime(2026, 3, 25, tzinfo=timezone.utc)  # Earlier date
    expiry_date = target_date + timedelta(days=5)  # 2026-03-30
    
    print(f"Running discovery for {target_date.date()}, expiry on {expiry_date.date()}")
    
    # Don't clear - use existing predictions
    ...
    # Check price data availability for expiry date
    print(f"\nChecking price data for {expiry_date.date()}:")
    liquid_predictions = []
    ...
    for p in predictions:
        ...
        # Also check a few days after expiry
        price_check_plus = repo.conn.execute("""
            SELECT COUNT(*) FROM price_bars 
            WHERE tenant_id = 'ml_train' AND ticker = ? AND DATE(timestamp) BETWEEN ? AND ?
        """, (p['ticker'], expiry_date.date(), (expiry_date + timedelta(days=3)).date())).fetchone()[0]
        
        print(f"  {p['ticker']}: {price_check} bars on {expiry_date.date()}, {price_check_plus} bars within 3 days")
    ...
    # Run replay on liquid predictions
