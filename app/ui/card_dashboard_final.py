"""
Final Minimal Card River Dashboard
Clean architecture with single data fetch layer and pure UI rendering
"""
...
class DashboardInputs:
    """Consolidated dashboard input state"""
...
def render_controls(service: DashboardService) -> Tuple[DashboardInputs, bool]:
    ...
    with st.sidebar:
        ...
        if not tenants:
            st.warning("⚠️ No tenants found in DB.")
...
def render_system_status(service: DashboardService, inputs: DashboardInputs) -> None:
    """Render consolidated system status row"""
    if not inputs.tenant:
        return
...
# ============================================================================
# CARD RIVER RENDERING
# ============================================================================

def render_cards(cards: List[Card], visible_count: int = 10) -> None:
    """Render cards as river with lazy loading"""
    if not cards:
...
def main():
    ...
    render_system_status(service, inputs)
