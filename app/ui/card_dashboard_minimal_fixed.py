"""
Minimal Card River Dashboard - Fixed Version
...
class MinimalCardProvider:
    ...
    def _get_best_picks(self, context: Dict) -> List[Card]:
        ...
        for i in range(10):
            ...
            series.append(SeriesPoint(forecast_date, base_value + i*0.3 + 2, "confidence_upper"))
            series.append(SeriesPoint(forecast_date, base_value + i*0.3 - 2, "confidence_lower"))
        
        cards.append(Card("chart", "NVDA - Top Pick", {
            "series": [vars(p) for p in series]
        }))
...
class MinimalCardRenderer:
    ...
    def render_chart_card(card: Card) -> None:
        ...
        if "comparison" in series_by_kind:
            ...
            # Create labeled groups for comparison
            comparison_labels = {}
            for p in series_by_kind["comparison"]:
                label = p.get("label", f"Series_{len(comparison_labels)}")
                if label not in comparison_labels:
                    comparison_labels[label] = []
                comparison_labels[label].append(p)
            
            for i, (label, points) in enumerate(comparison_labels.items()):
                if points:
                    fig.add_trace(go.Scatter(
                        x=[p["x"] for p in points],
                        y=[p["y"] for p in points],
        ...
        if "constituent" in series_by_kind:
            constituent_colors = [COLORS['neutral_400'], COLORS['neutral_500'], COLORS['neutral_600']]
            # Create labeled groups for constituents
            constituent_labels = {}
            for p in series_by_kind["constituent"]:
                label = p.get("label", f"Series_{len(constituent_labels)}")
                if label not in constituent_labels:
                    constituent_labels[label] = []
                constituent_labels[label].append(p)
            
            for i, (label, points) in enumerate(constituent_labels.items()):
                if points:
                    fig.add_trace(go.Scatter(
                        x=[p["x"] for p in points],
                        y=[p["y"] for p in points],
                        mode='lines',
                        name=label,
                        line=dict(color=constituent_colors[i % len(constituent_colors)], width=1, dash='dash'),
                        connectgaps=False
                    ))
...
class MinimalCardRiverDashboard:
    ...
    def render(self):
        ...
        cards = self.get_cached_cards(controls["view"], controls["strategy"], controls["horizon"])
