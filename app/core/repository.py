...
class Repository:
    ...
    def persist_prediction(self, prediction: Any, tenant_id: str = "default") -> None:
        ...
        self.execute(
            """
            INSERT OR REPLACE INTO predictions (id, tenant_id, strategy_id, scored_event_id, ticker, timestamp, prediction, confidence, horizon, entry_price, mode, feature_snapshot_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
