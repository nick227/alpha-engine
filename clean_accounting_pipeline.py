"""
Clean Accounting Pipeline

Fix reporting layer with canonical trade unit and correct concentration calculations.
"""
...
class CleanAccountingPipeline:
    """
    Clean accounting with position-level trade units.
    """
    ...
    def load_data(self):
        ...
        print("Loading historical data for clean accounting...")
    ...
    def generate_position_level_trades(self):
        """
        Generate position-level trades with canonical trade unit.
        """
        
        print("Generating position-level trades with canonical accounting...")
        ...
        position_trades = []  # Completed position-level trades
        ...
        for current_date in trading_days:
            ...
            for pos_id, position in positions.items():
                ...
                # Track excursions
                position['max_favorable_excursion'] = max(position['max_favorable_excursion'], current_r)
                position['max_adverse_excursion'] = min(position['max_adverse_excursion'], current_r)
            ...
            for pos_id, exit_price, exit_reason, current_r in positions_to_close:
                ...
                # Create canonical position-level trade
                position_trade = {
                    'trade_id': len(position_trades) + 1,
                    'ticker': position['ticker'],
        ...
        print(f"Generated {len(position_trades)} position-level trades")
        
        return position_trades
    
    def calculate_clean_metrics(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Calculate metrics from canonical trade dataframe.
        """
    ...
    def run_clean_accounting(self):
        """
        Run clean accounting with canonical trade units.
        """
        
        print("Clean Accounting Pipeline")
        print("Canonical trade units + correct concentration calculations\n")
        ...
        # Generate position-level trades
        trades = self.generate_position_level_trades()
        
        # Calculate clean metrics
        metrics = self.calculate_clean_metrics(trades)
        
        # Print results
        self.print_clean_results(metrics)
        
        return metrics
    
    def print_clean_results(self, metrics: Dict[str, Any]):
        """
        Print clean metrics with sanity checks.
        """
        
        print(f"\n=== CLEAN ACCOUNTING RESULTS ===")
        ...
        if metrics['top_5_contribution'] <= 50:
            print("  ✅ Top 5 contribution ≤ 50%: CONTROLLED")
            criteria_met += 1
        else:
            print(f"  ❌ Top 5 contribution ≤ 50%: {metrics['top_5_contribution']:.1f}% (too high)")
        
        if metrics['total_return'] > 0:
            print(f"  ✅ Return positive: {metrics['total_return']:+.1%}")
            criteria_met += 1
        else:
            print(f"  ❌ Return positive: {metrics['total_return']:+.1%}")
        
        if metrics['median_pnl'] > 0:
            print(f"  ✅ Median P&L positive: ${metrics['median_pnl']:,.0f}")
            criteria_met += 1
        else:
            print(f"  ❌ Median P&L positive: ${metrics['median_pnl']:,.0f}")
        
        if sector_total >= 95 and sector_total <= 105:
            print(f"  ✅ Sector contributions sum: {sector_total:.1f}% (reasonable)")
            criteria_met += 1
        else:
            print(f"  ❌ Sector contributions sum: {sector_total:.1f}% (should be 100%)")
        ...
        if criteria_met >= 3:
            print("🎯 CLEAN ACCOUNTING VALIDATED")
            print("   Metrics are trustworthy for strategy assessment")
        else:
            print("❌ ACCOUNTING ISSUES REMAIN")
            print("   Further debugging needed")


def main():
    """Main clean accounting function"""
    
    cleaner = CleanAccountingPipeline()
    metrics = cleaner.run_clean_accounting()
    
    return cleaner, metrics


if __name__ == "__main__":
    cleaner, metrics = main()
