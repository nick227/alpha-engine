...
def multi_day_momentum_signals(df, lookback_days=5, hold_days=3, threshold=0.03):
    """Generate multi-day momentum signals"""
...
def time_shift_test(signals, shift_days=2):
    """Time-shift test to detect leakage"""
    
    print(f"=== TIME-SHIFT TEST (shift {shift_days} days) ===")
    ...
    for date in sorted(signals_by_date.keys()):
        ...
        for signal in day_signals:
            trade_return = signal['trade_return'] - 0.0015  # Apply friction
        ...
        for signal in day_signals:
            # Find shifted signal
            shifted_signals = [s for s in signals if s['date'] == shifted_date and s['ticker'] == signal['ticker']]
            if shifted_signals:
...
def validate_momentum_edge():
    """Comprehensive validation of momentum edge"""
    
    print("=== COMPREHENSIVE MOMENTUM VALIDATION ===")
    print("Testing 5d and 10d momentum with 3d hold\n")
    
    # Get price data
    df = get_price_data()
    
    # Test both signals
    signals_5d = multi_day_momentum_signals(df, 5, 3, 0.03)
    signals_10d = multi_day_momentum_signals(df, 10, 3, 0.03)
    
    print(f"5d momentum: {len(signals_5d)} signals")
    print(f"10d momentum: {len(signals_10d)} signals")
    
    # Validate each
    for signal_name, signals in [("5d", signals_5d), ("10d", signals_10d)]:
        print(f"\n{'='*50}")
        print(f"VALIDATING {signal_name.upper()} MOMENTUM")
        print(f"{'='*50}")
        ...
        # Step 1: Time-shift test
        normal, shifted, passes = time_shift_test(signals)
        if not passes:
            print(">>> FAILED: Time-shift test indicates leakage")
            continue
        
        # Step 2: Yearly breakdown
        yearly_data = yearly_breakdown(signals)
        
        # Step 3: Overlap control
        ...
        if dist_stats['top5_contribution'] < 30:
        ...
        # Overall verdict
        if (passes and win_rate > 0.54 and total_return > 0 and dist_stats['top5_contribution'] < 30):
            print(f"\n>>> VERDICT: REAL EDGE DETECTED")
        elif passes and win_rate > 0.52:
            print(f"\n>>> VERDICT: PROMISING - NEEDS REFINEMENT")
        else:
            print(f"\n>>> VERDICT: NO EDGE")

if __name__ == "__main__":
    validate_momentum_edge()
